...
(`.deb` file) using the latest version of [Debian Bullseye](https://www.debian.org/releases/bullseye/).
...
jobs:
  build-deb:
    ...
    steps:
      ...
      - uses: singingwolfboy/build-dpkg-bullseye@v1
        id: build
        with:
...
This Action wraps the [`dpkg-buildpackage`](https://manpages.debian.org/bullseye/dpkg-dev/dpkg-buildpackage.1.en.html)
...
1. Call [`mk-build-deps`](http://manpages.ubuntu.com/manpages/bullseye/man1/mk-build-deps.1.html)
   to ensure that the build dependencies defined the `debian/control` file
   are installed in the Docker image.
2. Call [`dpkg-buildpackage`](https://manpages.debian.org/bullseye/dpkg-dev/dpkg-buildpackage.1.en.html)
